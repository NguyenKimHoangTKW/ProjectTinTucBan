    ...
    <div class="grid md:grid-cols-2 gap-6">
        <div>
            ...
            res.data.filter(muc => !muc.TenMucLuc?.toLowerCase().includes("thông báo")).forEach(muc => {
                ...
                if (isTinTuc) {
                    const sortedByDate = [...allBaiViets].filter(b => b.NgayDang).sort((a, b) => (b.NgayDang ?? 0) - (a.NgayDang ?? 0));
                    const sortedByViews = [...allBaiViets].sort((a, b) => (b.LuotXem ?? 0) - (a.LuotXem ?? 0));
                    const leftItems = sortedByDate.slice(0, 4);
                    const rightItems = sortedByViews.slice(0, 6);
                ...
                if (!allBaiViets.length) {
                    ...
                    html += `<div id="muc-${mucId}">
                        ...
                        <div class="text-right mt-4 space-x-2">                    
                            <button data-id="${mucId}" class="btn-an-bot hidden inline-flex items-center gap-2 px-4 py-2 border border-gray-600 text-gray-600 hover:border-red-700 hover:bg-red-50 hover:text-red-700 rounded-lg transition" title="Ẩn bớt">
                                <i class="fas fa-chevron-up text-blue-600 text-base"></i>
                            </button>
                            <button data-id="${mucId}" class="btn-xem-them inline-flex items-center gap-2 px-4 py-2 border border-red-600 text-red-600 hover:border-red-700 hover:bg-red-50 hover:text-red-700 rounded-lg transition" title="Xem thêm">
                    ...
                    if (isSuKien) {
                        ...
                        <a href="/danh-sach-bai-viet?mucId=${muc.ID}&slug=${toSlug(stripHtml(muc.TenMucLuc))}" class="inline-flex items-center gap-2 px-4 py-2 border border-yellow-700 text-yellow-700 hover:border-red-700 hover:bg-red-50 hover:text-red-700 rounded-lg transition" title="Xem tất cả sự kiện">
        ...
        error: function () {
            $("#mucLucContainer").html("<p class='text-center text-red-500'>Lỗi khi tải dữ liệu mục lục.</p>");
        }
    });

        // Load danh sách Khối và Đơn vị trực thuộc
    $.ajax({
        url: "/api/v1/home/get-khoi-va-donvi",
        method: "GET",
        dataType: "json",
        success: function (res) {
